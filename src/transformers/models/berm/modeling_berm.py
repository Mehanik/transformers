...
class BermForMaskedLM(BermPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.roberta = BermModel(config, add_pooling_layer=False)
...
class BermModel(BermPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.poolBERM_ = BermPooler(config) if add_pooling_layer else None
...
class BermMatrixLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        if config.hidden_size % config.num_matrix_streams != 0:
            raise ValueError(
                f"The hidden size ({config.hidden_size}) is not a multiple of the number of attention "
                f"heads ({config.num_matrix_streams})"
            )

        self.hidden_size = config.hidden_size
        self.num_matrix_streams = config.num_matrix_streams
        self.matrix_dim = int(config.hidden_size / config.num_matrix_streams)
        self.all_matrix_size = self.num_matrix_streams * self.matrix_dim * self.matrix_dim
    ...
    def forward(
        ...
        a = a.flatten(1, 2)  # merge batch and heads dims

        if past_vector is None:
            v = torch.zeros(a.size(1), self.hidden_size)
        ...
        for i in range(a.size(0)):
            ...
            if attention_mask is not None:
                v = new_v * attention_mask[i] + v * (1 - attention_mask[i])
            else:
                v = new_v

        context_layer = v.view(hidden_states.size())
...
class BermLayer(nn.Module):
    def __init__(self, config):
        ...
        assert self.add_cross_attention is None
    ...
    def forward(
        ...
        encoder_hidden_states: Optional[torch.FloatTensor] = None,
        encoder_attention_mask: Optional[torch.FloatTensor] = None,
        past_vector: Optional[Tuple[Tuple[torch.FloatTensor]]] = None,
        output_matrices: Optional[bool] = False,
    ) -> Tuple[torch.Tensor]:
        assert encoder_hidden_states is None, "Not implemented"
        assert encoder_attention_mask is None, "Not implemented"
